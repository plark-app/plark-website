...
export async function render(params: RenderParams): Promise<RenderResult> {
  ...
  logger.debug(
    `Critical CSS size: ${criticalCss.length}, location: ${location}`
  );
...
async function loadDependencies(
  ...
  resolved.forEach(
    (result: React.ComponentType, i: number) =>
      (context.components[keys[i]] = result)
  );
}

async function compressCss(css: string): Promise<string> {
  return new Promise(
    (res: (str: string) => void, rej: (err: Error) => void) => {
      // tslint:disable-next-line:no-any
      cssnano
        .process(css, { from: undefined })
        .then((r: any) => res(r.toString()), rej);
    }
  );
}
